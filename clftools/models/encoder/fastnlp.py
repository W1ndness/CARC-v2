...
class FastNLPEncoder:
    ...
    def static_embed(self, sentence, embed_size, model_name='cn-fasttext'):
    ...
    def elmo_embed(self, sentence, model_name, requires_grad=False, layers=None):
        self.embed_size = 256
    ...
    def bert_embed(self, sentence, include_cls_sep=False, layers='-1', model_name='cn-wwm-ext'):
        self.embed_size = 768
        if include_cls_sep and layers is not None:
            self.embed_size *= len(layers.split(','))
        self.embed = BertEmbedding(self.vocab,
                                   model_dir_or_name=model_name,
